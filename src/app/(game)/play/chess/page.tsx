...
import type { ChessState } from "@/lib/chess";

interface Props {
  searchParams: Promise<{ game_id?: string }>;
}

export default async function ChessPage({ searchParams }: Props) {
  ...
  if (!game || game.game_type !== "chess") redirect("/lobby");
  ...
  const initialState: ChessState = (raw && "board" in raw)
