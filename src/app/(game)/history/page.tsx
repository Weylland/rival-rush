...
export default async function HistoryPage() {
  ...
  return (
    <div style={{ position: "relative", minHeight: "100dvh", background: RR.violet, overflow: "hidden" }}>
      ...
      <SvgBlob color={RR.cyan} style={{ width: 500, height: 440, top: -160, right: -130, opacity: 0.65, animation: "rr-float 7s ease-in-out infinite" }} />
      <SvgBlob color={RR.butter} style={{ width: 420, height: 380, bottom: -150, left: -120, opacity: 0.5, animation: "rr-float 9s ease-in-out infinite reverse" }}
        path="M 50 30 Q 90 5 140 30 Q 195 50 180 110 Q 175 175 110 175 Q 30 180 25 120 Q 10 60 50 30 Z" />

      <Star color={RR.butter} size={36} style={{ top: "7%", left: "5%", animation: "rr-spin-slow 12s linear infinite" }} />
      <Star color={RR.white} size={22} style={{ top: "5%", right: "7%", animation: "rr-spin-slow 18s linear infinite reverse" }} />
      <Star color={RR.pink} size={18} style={{ bottom: "24%", right: "5%", animation: "rr-float 5s ease-in-out infinite" }} />
      <Star color={RR.cyan} size={14} style={{ bottom: "10%", left: "8%", transform: "rotate(-15deg)" }} />

      <div style={{ position: "relative", zIndex: 10, maxWidth: 680, margin: "0 auto", padding: "40px 24px 100px" }}>
        {/* Header */}
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: "var(--font-sans)", fontSize: 13, fontWeight: 900, color: RR.cyan, textTransform: "uppercase", letterSpacing: 2 }}>
            {session.pseudo}
          </div>
          <div style={{ fontFamily: "var(--font-display)", fontSize: 48, color: RR.white, transform: "skewX(-8deg)", textShadow: `3px 3px 0 ${RR.pink}`, marginTop: 4 }}>
            HISTORIQUE
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {rows.map((game) => {
            ...
            const resultColor = isDraw ? RR.butter : isWin ? RR.cyan : RR.pink;
            ...
            return (
              <div key={game.id} style={{
                ...
                {/* Result badge */}
                <div style={{
                  background: resultColor, border: `2px solid ${RR.ink}`,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <Avatar name={opponentPseudo} src={game.opponent_avatar_url} color={game.opponent_avatar_color ?? RR.pink} ring="transparent" size={28} />
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 15, color: RR.white, transform: "skewX(-4deg)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{
                      background:
                        game.game_type === "pfc"        ? RR.cyan :
                        game.game_type === "puissance4" ? RR.butter :
                        game.game_type === "reflexe"    ? RR.pink :
                        game.game_type === "naval"      ? "#0ea5e9" :
                        game.game_type === "chess"      ? "#9b8ec4" :
                        game.game_type === "nim"        ? RR.butter :
                        game.game_type === "pig"        ? RR.pink :
                        game.game_type === "mastermind" ? "#4ADE80" :
                        RR.pink,
                      border: `1.5px solid ${RR.ink}`, borderRadius: 999,
                      padding: "2px 8px", fontSize: 10,
                      fontFamily: "var(--font-display)", color: RR.ink, letterSpacing: 0.6,
                    }}>
        ...
        <div style={{ marginTop: 40, textAlign: "center" }}>
          <Link href="/ranking" style={{
            fontFamily: "var(--font-display)", fontSize: 18, color: RR.cyan,
            textDecoration: "none", display: "inline-block", transform: "skewX(-4deg)",
            borderBottom: `2px solid ${RR.cyan}`, paddingBottom: 2,
          }}>
            ← CLASSEMENT
          </Link>
        </div>
